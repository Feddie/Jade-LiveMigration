...
public class LoadMonitor extends TickerBehaviour {
	public static final double REG_THR = 0.3; //CPUload threshold for DF registration
	...
	public void onTick() {
		...
		if (cpuload == -1.0) { //error in reading CPU Load
			...
			if (this.cpuload >= LoadMonitor.REG_THR ) { //Deregister from DF as a migration site	
				...
				if (this.cpuload > LoadMonitor.MIGR_THR && !((Teleporter) myAgent).migrating) { //with a high CPU load, try to perform a migration	
					if (!runvms.isEmpty()) { 
